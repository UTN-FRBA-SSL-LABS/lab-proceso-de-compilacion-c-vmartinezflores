...
> **R:** Porque el prepocesador expande los #include y macros, insertando el contenido completo de los headers del sistema.
...
> **R:**Porque, en el ´programa.i´, no existe el patrón mencionado.
...
> **R:** Son marcadores de linea que comunica la información del origen de cada bloque, el nombre y la linea del archivo original. La declaración printf proviene de stdio.h
...
> **R:** "LFB14:" Etiqueta interna que marca el comienzo del bloque de función. ".cfi_startproc" directiva que indica al compilador que empieza un procedimiento. "pushl	%ebp" guarda el valor actual del puntero de base en la pila. ".cfi_def_cfa_offset 8" directiva de depuración que define como calcular la dirección base del marco de pila.
...
> **R:** area_circulo aparece con otro tipo distinto en programa.o porque allí solo se la declara y llama, mientras que en matematica.o se la define.
...
> **R:** Porque son funciones de la biblioteca dinámica del sistema.El cargador dinámico las resuelve cuando el programa se ejecute.
...
> **R:** Una macro función como CUADRADO(x) se sustituye en el preprocesamiento antes de la compilación, se reemplaza por el codigo que corresponda y no tiene verificación de tipos por lo que si pasa algo que no sea numero, igual se reemplaza. En cambio, una función real como sumar(a, b) se resuelve en el enlazado y si tiene verificación de tipos.
...
> **R:** Simbolo de tipo "T" esta definido en la sección de text del archivo objeto, codigo ejecutable como area_circulo. El simbolo de tipo "D" esta definido en la seccion de data, variable global inicializada.
...
> **R:**  ntdll.dll => /c/WINDOWS/SYSTEM32/ntdll.dll (0x7ffb2ec80000)
        ntdll.dll => /c/Windows/SysWOW64/ntdll.dll (0x77140000)
        wow64.dll => /c/WINDOWS/System32/wow64.dll (0x7ffb2ced0000)
        wow64base.dll => /c/WINDOWS/System32/wow64base.dll (0x7ffb2d770000)
        wow64win.dll => /c/WINDOWS/System32/wow64win.dll (0x7ffb2e280000)
        wow64con.dll => /c/WINDOWS/System32/wow64con.dll (0x7ffb2e310000)


¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque "libc" existe en ambos tipos, estática y dinámica, asi que por defecto gcc usa la versión dinámica.
